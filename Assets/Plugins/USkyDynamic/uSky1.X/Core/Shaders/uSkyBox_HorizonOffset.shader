...
SubShader 
	{
		...
		#ifdef NIGHTSKY_ON
			float2	moonTC			: TEXCOORD1;
			half	nightConst		: TEXCOORD2;
			float3	spaceTC			: TEXCOORD3;
		#endif
		};
		
		uniform half 		_SunSize, _OuterSpaceIntensity,_uSkyHorizonOffset;
...
//			OUT.nightConst = OUT.zenithAngle.w * _SkyMultiplier.z ; 
			OUT.nightConst = max(0.0, OUT.worldPos.y ) * _SkyMultiplier.z ; 
		...
		half4 frag(v2f IN) : SV_Target
		{
			...
			half t = pos.y + _uSkyHorizonOffset;
			...
			// add sun
			if ( _SunDir.y > -0.1 ){
			half sun = min(6e2, pow((1-cosine)* _SunSize, -1.5 ));
...
// --------------------------------------------------------------------------------			
	...
	Fallback "uSky/uSkyBox Simplified SM2"
}